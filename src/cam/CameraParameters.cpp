...
{
	//CameraParameters::Write(path_and_file); // for testing

	if (!FileUtils::Exists(path_and_file)) {
		cout << "\n[ERROR] - Cannot find file " << path_and_file << "." << endl;
		return false;
	}
	...
	if(!fs.isOpened()) {
		cout << "\n[ERROR] - Cannot open file " << path_and_file << "." << endl;
		return false;
	}
...
{
	if (intrinsic_matrix.at<float>(0,0) == 0) {
		intrinsic_matrix.at<float>(0,0)= 590.0;
		intrinsic_matrix.at<float>(1,1)= 590.0;
		intrinsic_matrix.at<float>(2,0)= 480.0/2.0;
		intrinsic_matrix.at<float>(2,2)= 640.0/2.0;
	}
...
{
	if (!parameters_ready) {
		cout << "\n[ERROR] - Cannot calculate FoV. Load camera parameters first. " << endl;
